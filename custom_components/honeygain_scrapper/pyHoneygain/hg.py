...
class HoneyGain:
    def __init__(self, API_PREFIX_URL: str = "/api", API_VERSION: str = "/v1", API_DOMAIN: str = "https://dashboard.honeygain.com") -> None:
        ...
        self.API_BASE_URL = f'{self.API_DOMAIN}{self.API_PREFIX_URL}'
        self.jwt = None
        self.user_id = None
        self.remove_proxy()

    def __make_request(self, req_type: str, endpoint: str, headers: dict = {}, api_version = None, *args, **kwargs):
        """Helper function to make requests. """
        if api_version == None:
            api_version = self.API_VERSION

        try:
            return requests.request(req_type, f'{self.API_BASE_URL}{api_version}{endpoint}', proxies=self.proxy_conf, headers={
                **headers, **({
                    "Authorization": f"Bearer {self.jwt}",
                } if self.jwt is not None else {}),
            }, *args, **kwargs)
        except:
            return requests.request(req_type, f'{self.API_BASE_URL}{self.API_VERSION}{endpoint}', proxies=self.proxy_conf, headers={
                **headers, **({
                    "Authorization": f"Bearer {self.jwt}",
                } if self.jwt is not None else {}),
            }, *args, **kwargs)
    ...
    def devices(self, api_version = None):
        """Returns devices info. As of 16th Feb 2022, /v1 and /v2 both return different data."""
        self.handle_not_logged_in()

        r = self.__make_request("GET", "/devices", api_version=api_version)
    ...
    def notifications(self):
        ...
        r = self.__make_request(
            "GET", f"/notifications?user_id={self.user_id}")
